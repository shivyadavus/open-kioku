...
| Secret exposure through indexing | Sensitive files become searchable | Credential-shaped paths (`.env*`, `.aws`, `.ssh`, key files) are always denied and hidden files are blocked by default; other files are indexed even when named `secrets` or `credentials`: data, config and prose files with secret-like values replaced by `[REDACTED]` before storage (`docs/security-model.md`, "Secret-value redaction"), programming-language files as written with parser messages redacted. |
