...
- `ok snapshot export` refuses `--quality fast` while an index still owes this clearing, because that mode copies the database file as it is, free pages included; `--quality best` rewrites the database through `VACUUM INTO` and is the default.
