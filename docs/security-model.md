...
- An index written before redaction stored these files' values as read, and SQLite keeps the bytes of deleted rows in free pages until they are reused. The first `ok index` over such an index (its manifest has no `redacted_files`) runs `VACUUM` after publishing the new manifest, then truncates the WAL, so those pages are dropped; `ok watch` rebuilds such an index in full rather than updating it in part, and compacts it the same way. This happens once per index. It rewrites the whole database, takes time proportional to the database's size, and needs free disk space about that size while it runs; if another process holds the database open, the WAL truncation completes at SQLite's next checkpoint, and if the compaction itself cannot run the index run reports it and leaves those pages for the next run (the manifest is published either way, so this never fails an index). `ok snapshot export` compacts through `VACUUM INTO` by default; `--quality fast` copies the database file as it is.
