...
| `find_tests_for_change` | What should I run now? | `path` (required to select tests) |
...
- `find_tests_for_change` returns ranked test file paths with relevance scores from naming conventions, import relationships, and co-change history. A `path` is required to select tests: without one the response selects none and its caveat says no changed path was given. It executes nothing, and a ranked test is a candidate, not proof of coverage. The response is `{tests, excluded, excluded_sample, caveats}`, and `ok --json tests --changed <path>` returns the same object from the same selector (`TestSelector::select_for_changed_path`). `tests` holds only targets that can stand as validation evidence. A target matched to the change that cannot — today, a registration the runner skips (`test.skip`, `it.todo`, `test.failing`), reason `disabled` — is never listed there: `excluded` counts such targets by reason and `excluded_sample` names up to five of them (`id`, `name`, `path`, `line`, `reason`), ordered by file and line. Both are omitted when nothing was withheld. Whenever `tests` is empty, `caveats` says why, and the two cases read differently: `no indexed test target was found for <path>` when nothing matched, and `N indexed test target(s) found for <path>, all excluded as validation evidence (…)` with the step that would make them count when everything matched was withheld. `ok status --json` and `repo_status` carry the repository-wide count as `quality.excluded_test_targets`, beside `quality.test_count`, which counts only targets that stand as evidence; it is `{}` when nothing was excluded and absent on an index written before it was recorded, which `ok setup audit` reports as needing a re-index to tell skipped tests from absent ones. With `limit: 0` an empty `tests` says how many runnable targets matched rather than claiming none were found.
