...
Every language × relationship cohort has a metamorphic group. Variants are indexed independently from the same logical source state. The hardened variants add unrelated source and reverse indexed evidence vectors before graph construction, exercising order independence rather than comparing only final pass/fail verdicts.
...
Clean rebuild equivalence is inherent in independently indexing each variant. Existing incremental-index tests remain responsible for path-level storage/update mechanics; the release conformance gate focuses on the resulting RI3 relationship truth. Any future incremental conformance fixture added here must compare that same canonical relationship/proof identity.
