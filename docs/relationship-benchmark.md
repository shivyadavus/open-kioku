...
- **Must not emit:**
  ...
  - `crate::` reaching another workspace member's same-text module, a module only the root package of a workspace declares, from a `src/bin/` crate root the library's module, or from a file an integration test mounts with `#[path]` another integration test crate's item;
