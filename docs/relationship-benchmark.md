...
`benchmarks/relationship-ci-cases.json` is the compact subset used by normal CI: one case per cohort plus targeted regression cases. The Rust `CALLS` regressions (`ci-rust-calls-02` to `-53`) write multi-file packages, and the live producer fails a case if any of its `.rs` files was not indexed. `-18` to `-25` cover inline `mod` blocks: an item or type of the file is in scope inside a `mod` block only through `use super::*`, `use super::name` or another import, an item the block declares needs neither, and a same-named item or type elsewhere in the file must not take the edge; `-25` keeps the file's own item reachable through `use super::*` beside a second glob. `-26` and `-27` check that a bare call inside a method never names an associated function of the enclosing `impl`. `-28` to `-33` cover `self::` and `super::` paths written inside inline `mod` blocks: each `super` leaves the innermost block, so `super::target_fn()` in `mod tests` names the file's own item and never the crate root's, `super::super::` climbs past the file to the crate root, a nested block reaches its enclosing block's item, and a path landing on a module that only imports the name emits no authoritative edge. `-34` to `-36` check that a path through `mod name;` continues into that module's file, from the file itself and from a block, and that `self::` in a block reaches an item its `use super::*` brings in. `-37` and `-38` set `forbid_heuristic_edges`: a `MustNotEmit` case normally counts only authoritative edges, and with this flag a non-authoritative edge from the source is a `must_not_emit_heuristic_violation` too. They check that the symbol-registry pass links neither a call under `use mock_clock::target_fn;` in a `mod` block nor a call in a sibling `mod` block to the file's same-named item, since those heuristic edges reach `get_references` callers and impact's possible impact. `-39` to `-41` cover `#[path]` modules on paths the resolver spells from file paths: `self::pathed::target_fn()` and `crate::w::pathed::target_fn()` must not reach the file left at the default location of `#[path = "elsewhere.rs"] mod pathed;`, and `super::target_fn()` in a file mounted by `#[path]` must not climb the directories of its file path. `-42` to `-46` cover the crate a module path is read against: in a workspace member of a root package, `crate::only_root::target_fn()` must not reach a module only the root package declares and `crate::util::target_fn()` reaches the member's own `util.rs`; `crate::x::target_fn()` in `src/bin/tool.rs`, a crate root of its own, must not reach the library's `x`; `crate::r#type::target_fn()` reaches `src/type.rs`; and with `[lib] path = "src/app_lib.rs"` that file is the crate root whose `mod a;` places `src/a.rs`. `-47` to `-49` cover crate roots Cargo discovers outside `src/lib.rs` and `src/main.rs`: `src/bin/multi/main.rs` keeps its modules in `src/bin/multi/`, `src/bin/tool.rs` keeps its `mod sub;` beside it in `src/bin/sub/mod.rs` and not in `src/bin/tool/sub.rs`, and an integration test's `mod common;` is `tests/common/mod.rs`. `-50` checks that `crate::target_fn()` in a file one integration test mounts with `#[path]` proves no edge into another integration test crate's `target_fn`. `-51` to `-53` cover crate roots the index does not read from file layout alone: with `src/main.rs` skipped by `.okignore` (the live producer fails the case unless discovery skips exactly that file), `crate::target_fn()` in the `src/cli.rs` only it declares must not reach the library's `target_fn`; `use crate::target_fn;` in `src/bin/tool/sub.rs`, which no crate root declares, must not bind the binary's `target_fn`; and `[[bin]] path = "src/cli.rs"` is a crate root whose `mod util;` places `src/util.rs`. `ci-python-calls-02` and `-03` cover a Python import made inside a `try:` body: alone it binds the call, and with an alternative import under `except ImportError:` the call keeps both candidates and no authoritative edge; the live producer fails either case if one of its `.py` files was not indexed. The Rust `IMPORTS` regressions (`ci-rust-imports-02` to `-04`) write a package whose crate root only declares the modules and re-exports one item: a cross-module item import and a glob must reach the file declaring what they name, and an item reachable only through the crate-root `pub use` must emit no authoritative edge.
- **Must emit:** a call through a cross-module item import, and its grouped, aliased twin in the same metamorphic group; a call through `crate::` inside the importing workspace member, with a same-text module in the other member; a `crate::` call path inside a workspace member of a root package with a same-text module in the root package; a `crate::` call path through a raw-identifier module (`r#type`) and from a crate root set by `[lib] path`; `self::` and `crate::` call paths into the modules of a `src/bin/` binary, an integration test, and a `[[bin]] path` crate root in `src/`.
- **Must not emit:**
  ...
  - paths the module tree does not support: `super::` inside an inline module, a relative import from a file mounted by `#[path]`, a stale file beside `#[path]`, `self::`, `crate::` and `super::` call paths that reach the default location of a `#[path]` module or start in a file mounted by one, a name that is both a submodule and a function, and `callee.rs` beside `callee/mod.rs` (rustc rejects that layout, but the index still has to fail closed on it);
  - `crate::` reaching another workspace member's same-text module, a module only the root package of a workspace declares, from a `src/bin/` crate root the library's module, from a file an integration test mounts with `#[path]` another integration test crate's item, from a file only a skipped `main.rs` declares the library's item, or from a binary-directory file no crate root declares that binary's item;
