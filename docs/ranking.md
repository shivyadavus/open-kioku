...
- Task respelling in the lexical stream (candidate generation, no score component of its own; it moves `retrieval_rrf:lexical` ranks). The lexical stream queries the whole task first, then its single words and word pairs, and ranks continue across those terms so a one-word hit never leapfrogs a whole-task hit. A few task words have a fixed code-vocabulary alias (`configuration`, `configurations`, `configured`, `configuring` → `config`; `defaults` → `default`; `histories` → `history`). When the task contains one, the whole task with those words replaced (`add history configuration defaults` → `add history config default`) is queried alongside the task as written. The best hits of both spellings, up to the one query's limit, share the first tier by BM25 score; every literal whole-task hit that tier displaced follows it in the literal query's order, so a respelling demotes a literal match but never removes it, and only respelled-only hits past the limit are left out (a respelling cannot double the whole task's share of the stream). A chunk reached by both spellings counts once, at its better score, and carries the evidence lines of both matches; a respelled hit carries ``task respelled in code vocabulary `…` matched indexed search``. This describes the Tantivy lexical source. The in-memory fallback used when no search index exists searches the respelling as one more task anchor: it merges results by path at their best score with no shared cap, and its evidence line reads ``task anchor `…` matched``, so it does not reproduce the tiering. Until this change the aliases were only queried as single words and pairs after every literal term, so they could never lift a file that matches the whole task in code spelling: for the workflow case `config-history`, the file defining `HistoryConfig` and `default_history_max_commits` was lexical rank 48, behind files matching one literal word, and held the plan's eighth slot by a score margin of 3e-4; it is now lexical rank 1 and first in the plan (0.2895 against 0.2718 for the second file). Unlike a lattice hop, an alias is a fixed abbreviation or plural of the task's own word rather than a guess, which is why it shares the whole task's tier instead of ranking after the literal terms. Tasks with no aliased word are unaffected. The alias table is small and hand-written; widening it (for example to general plural normalization) is a separate change that needs its own measurement. Measured on the commit-derived corpora (CI runner, one run each, against the frozen baselines): Java (10k files) holdout R@20 0.7080 → 0.7168, MRR 0.5160 → 0.5166, dev identical; Go (~800 files) dev R@5 0.5663 → 0.5714, MRR 0.3984 → 0.3995, holdout identical; TypeScript (~900 files) identical on both splits; Python (~4k files) holdout R@10 0.7236 → 0.7286, R@20 0.7538 → 0.7588, but R@1 0.4422 → 0.4372 and MRR 0.5426 → 0.5397 (one case of 199 left rank 1; in its gated `general` family R@1 0.4521 → 0.4384 and MRR 0.5403 → 0.5312), dev MRR unchanged to four places. Every gated family stayed within tolerance. The retrieval fixture baseline is bit-identical.
