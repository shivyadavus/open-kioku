...
To map a chunk-level id, match its `<path>` to the primary result or `selected_units` entry with the same `path`; `<start>-<end>` are lines of that file, which may lie outside what the pack shows. It identifies the index hit that scored the unit, not a line of the pack, so a consumer that needs a record resolves the result's own `evidence_refs` instead. Ids from other producers keep their producer's id here too: `history-*` ids and history fact ids resolve to pack records; `test:<id>` names the test target whose `id` is `<id>`, which is listed in `test_candidates` only when that target was selected; graph edge and symbol ids are the graph's own. `ok search` and MCP `search_code` publish no records, so every `search:` id on their results, including the ones in `evidence_refs`, names an index hit's chunk in the same way; a merged hybrid line whose index another line already held is renumbered under its chunk (`docs/mcp-tools.md`, `search_code`).
