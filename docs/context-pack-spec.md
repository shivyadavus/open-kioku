...
## Chunk-level evidence ids

The ids in a result's `score_breakdown[].evidence_ids` and in `retrieval_diagnostics.traces[].contributions[].evidence_refs` are not pack record ids, and most of them resolve to no entry in `evidence`. They record what each source returned before fusion, deduplication and region widening. A retrieval id there is chunk-level, `search:<path>:<start>-<end>:<index>`: `<start>-<end>` is the line range of the chunk the index hit, and `<index>` the position of the line in that hit's own evidence list. Records are published under the final result's `line_range` and line position (the `evidence_refs` pairing rules above), so a chunk-level id equals a record id only when the result kept the chunk's range and the line kept its position. Fusion merges every lexical and semantic hit on a code file into that file's unit, so one contribution can cite several chunks of the file, including chunks outside the unit's `line_range`.

`retrieval_diagnostics.traces` covers every fused candidate, not only the ones the pack selected, so most traces name a path that no primary result, `selected_units` entry or supporting result carries; the ids on such a trace map to nothing in the pack. For a selected unit, match the id's `<path>` to the primary result or `selected_units` entry with the same `path`; `<start>-<end>` are lines of that file, which may lie outside what the pack shows. It identifies the index hit that scored the unit, not a line of the pack, so a consumer that needs a record resolves the result's own `evidence_refs` instead. Ids from other producers keep their producer's id here too: on a primary result, `history-*` ids and history fact ids resolve to pack records, while on a supporting result, which publishes no records, they resolve to none; `test:<id>` names the test target whose `id` is `<id>`, which is listed in `test_candidates` only when that target was selected; graph edge and symbol ids are the graph's own. `ok search` and MCP `search_code` publish no records, so every `search:` id on their results, including the ones in `evidence_refs`, names an index hit's chunk in the same way; a merged hybrid line whose index another line already held is renumbered under its chunk (`docs/mcp-tools.md`, `search_code`).

The ids are left chunk-level on purpose: they are what makes a score reproducible from the index, and renaming them to the widened unit would claim that the whole unit scored, which it did not.
