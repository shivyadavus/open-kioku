...
The graph builder creates file-to-symbol `DEFINES` edges and heuristic symbol reference edges from indexed chunks. SQLite persists `graph_nodes` and `graph_edges`, and `open-kioku-storage::GraphStore` exposes neighborhood and shortest-path traversal to CLI and MCP callers.
