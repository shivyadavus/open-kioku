...
- Confirm `Cargo.toml` `[workspace.package]` version is `3.0.1`.
- Confirm `release-metadata.json` uses tag `v3.0.1`.
- Confirm the GitHub release tag is exactly `v3.0.1`.
- Confirm `CHANGELOG.md` has a `3.0.1` section and a matching `[3.0.1]` release link.
