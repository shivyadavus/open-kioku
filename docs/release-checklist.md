...
- Confirm `Cargo.toml` `[workspace.package]` version is `2.2.0`.
- Confirm `release-metadata.json` uses tag `v2.2.0`.
- Confirm the GitHub release tag is exactly `v2.2.0`.
- Confirm `CHANGELOG.md` has a `2.2.0` section and a matching `[2.2.0]` release link.
