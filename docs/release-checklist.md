...
- Confirm `Cargo.toml` `[workspace.package]` version is `3.0.2`.
- Confirm `release-metadata.json` uses tag `v3.0.2`.
- Confirm the GitHub release tag is exactly `v3.0.2`.
- Confirm `CHANGELOG.md` has a `3.0.2` section and a matching `[3.0.2]` release link.
