...
new = '''    #[serde(default)]
    ...
    pub retrieval_confidence: Option<Confidence>,'''
if text.count(old) != 1:
    raise SystemExit(f'core field marker count={text.count(old)}')
...
new = '''            ambiguity_unresolved_count: 0,
            ...
            retrieval_confidence: None,'''
if text.count(old) != 1:
    raise SystemExit(f'core default marker count={text.count(old)}')
text = text.replace(old, new, 1)
...
new = '''    let mut exact_paths = std::collections::BTreeSet::new();
    ...
    for trace in &diagnostics.traces {'''
if text.count(old) != 1:
    raise SystemExit(f'trace declaration marker count={text.count(old)}')
text = text.replace(old, new, 1)
...
new = '''        if !selected_paths.contains(&path) {
        ...
        if trace.authority == RetrievalAuthority::Exact {'''
if text.count(old) != 1:
    raise SystemExit(f'trace attribution marker count={text.count(old)}')
text = text.replace(old, new, 1)
...
# First matching renderer block is Markdown.
...
if text.count(old) < 2:
    raise SystemExit(f'renderer marker count={text.count(old)}, expected at least 2')
new = '''            diagnostics.selection.exact_evidence_count,
        ...
        if let Some(confidence) = diagnostics.selection.retrieval_confidence {'''
text = text.replace(old, new, 1)
# Remaining matching block is Prompt.
if text.count(old) < 1:
    raise SystemExit('prompt renderer marker missing after markdown patch')
new = '''            diagnostics.selection.exact_evidence_count,
            diagnostics.selection.ambiguity_unresolved_count
        ));
        if diagnostics.selection.unattributed_selected_file_count > 0 {
...
new = '''        assert_eq!(diagnostics.selection.source_stream_mix.len(), 2);
        ...
        assert_eq!(
            diagnostics'''
if text.count(old) != 1:
    raise SystemExit(f'attributed test marker count={text.count(old)}')
text = text.replace(old, new, 1)
...
if text.count(marker) != 1:
    raise SystemExit(f'unattributed test marker count={text.count(marker)}')
