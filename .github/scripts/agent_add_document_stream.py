...
# 1) Retrieval source taxonomy: first-class Document stream.
...
# 2) Candidate fusion/source plumbing: exclude docs from code lexical, add Document everywhere.
...
# 3) Built-in document stream: bounded Markdown/MDX chunks with heading + line provenance.
...
document_fn = '''    fn document_stream(&self, request: &CandidateRequest) -> CandidateStream {
        ...
        let mut scored = self
            ...
            .filter_map(|chunk| {
                ...
                let heading_path = document_heading_path(&chunk.text);
                let haystack = format!(
                    "{} {} {}",
                    file.path.display(),
                    heading_path,
...
fn document_heading_path(text: &str) -> String {
...
new_tests = '''    #[test]
    ...
    #[test]
    fn document_heading_path_is_deterministic_and_nested() {
        let text = "# Context Compiler\nintro\n## Retrieval\ntext\n### Documents\nmore";
        assert_eq!(
            document_heading_path(text),
            "Context Compiler > Retrieval > Documents"
        );
    }
'''
if text.count(marker) != 1:
...
# 4) Benchmark document stream independently with all other CC2 streams.
