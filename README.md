...
- **The index reports what it did not index.** Per-language coverage with every omission attributed to a skip reason, in `ok index`, `ok doctor`, `ok status`, and `repo_status`; files a policy excluded (hidden, ignored, vendored) are reported beside the ratio with the setting that governs them, not counted as missing. An ingest rule had silently dropped 25 Java source files from one repository.
