...
- **The index reports what it did not index.** Per-language coverage with every omission attributed to a skip reason, in `ok index`, `ok doctor`, `ok status`, and `repo_status`. An ingest rule had silently dropped 25 Java source files from a 10k-file Java service.
