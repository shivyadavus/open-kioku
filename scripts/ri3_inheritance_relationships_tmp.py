...
inheritance.write_text(r'''use crate::context::{ResolutionContext, ResolutionResult, UnresolvedReason};
...
mod tests {
    ...
    fn build_splits_only_top_level_parent_commas() {
        ...
        assert_eq!(
            edges.iter().map(|edge| edge.parent_name.as_str()).collect::<Vec<_>>(),
            vec!["Alpha", "Beta<Map<Key, Value>>", "Gamma"]
        );
        assert_eq!(edges.iter().map(|edge| edge.order).collect::<Vec<_>>(), vec![0, 1, 2]);
    }
...
tests = '''    #[test]
    ...
    fn ambiguous_inheritance_target_cannot_authorize_extends() {
        ...
        assert_ne!(ambiguous.relationship_authority(), RelationshipAuthority::Authoritative);
    }

'''
text = replace_exact(text, anchor, tests + anchor, "inheritance authority tests")
...
insert = '''            quality_report.call_sites = call_sites.len();
            ...
            for binding in &bindings {
'''
text = replace_exact(text, anchor, insert, "inheritance relationship ingest emission")
...
''',
    "resolved relationship evidence range fallback",
)
