...
def main():
    ...
    ap.add_argument(
        ...
        help="keep every commit whose subject repeats an earlier one up to numbers (default: keep the first only; go-a's 'releaser: Bump versions for release of X' was a third of its holdout and every instance has the same gold file, so one pattern decided the corpus)",
    )
    ap.add_argument(
