...
/// directory that `.gitignore` also lists counts as hidden: ingest checks the hidden
/// rule first, so the doctor's git-ignore warning cannot fire on agent worktrees.
#[test]
