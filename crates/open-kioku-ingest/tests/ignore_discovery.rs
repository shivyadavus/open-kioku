...
fn nested_gitignore_is_scoped_to_its_directory() {
    ...
    assert!(indexed.contains(&Path::new("notes/README.md")));
...
fn tracked_file_is_not_skipped_when_gitignore_pattern_matches_it() {
    ...
    let tracked_check = Command::new("git")
        ...
        .unwrap();
    assert!(
        !tracked_check.success(),
        "Git must not report the tracked file as ignored"
    );
...
#[test]
fn zero_index_after_ignore_filter_emits_discovery_warning() {
    let dir = initialized_repo();
    write(dir.path(), ".gitignore", "*.java\n");
    write(dir.path(), "src/Foo.java", "class Foo {}\n");
    ...
    let scan = snapshot
        .phase_reports
        .iter()
        .rfind(|report| report.phase == "scan")
        .expect("final scan phase report");
    assert!(scan
        .warnings
        .iter()
        .any(|warning| warning.contains("indexed no supported files")));
}
