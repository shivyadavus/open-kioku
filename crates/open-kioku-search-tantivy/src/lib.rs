...
impl SearchIndex for TantivySearchIndex {
    ...
    fn search(&self, query: &str, limit: usize) -> Result<Vec<SearchResult>> {
        ...
        let query = parser
            .parse_query(query)
            .map_err(|err| OcfError::Search(err.to_string()))?;
...
fn required_json<T: serde::de::DeserializeOwned>(
    ...
    let value = document
        ...
        .ok_or_else(|| OcfError::Search("tantivy document is missing stored JSON".into()))?;
...
fn search_err(err: tantivy::TantivyError) -> OcfError {
    OcfError::Search(err.to_string())
}
