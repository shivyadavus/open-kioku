...
fn policy_for(family: TaskFamily, query_shape: QueryShape) -> RetrievalPolicy {
    ...
    let mut policy = match family {
        ...
        // source files (`docs(expect): fix typo on objectContaining example` in ts-a targets
        // `yaml/yaml.ts`), and a documentation task that can only see Markdown sections
