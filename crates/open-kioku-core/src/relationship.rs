...
//!
//! This module owns the serialized proof vocabulary shared by graph storage, query, and
//! downstream consumers. Relationship-specific authorization policy intentionally lives above
//! core so parsers and storage do not make trust decisions independently.
...
///
/// Ordering is deliberate so callers can compare minimum authority without translating values.
#[derive(
    Debug,
    Clone,
    Copy,
...
impl RelationshipProofKind {
    /// Maximum authority this proof kind can contribute before relationship-specific policy is
    /// evaluated. This is an intrinsic safety ceiling, not an edge authorization decision.
...
impl RelationshipProof {
    ...
    /// Canonicalize set-like fields and cap untrusted serialized authority at the proof-kind ceiling.
...
mod tests {
    ...
    fn proof_normalization_is_deterministic() {
        ...
        assert_eq!(proof.evidence_ids, vec![EvidenceId::new("a"), EvidenceId::new("z")]);
    }
}
