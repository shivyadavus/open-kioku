...
- `open_kioku_core::SyntaxFacts` gains the public field `module_declarations: Vec<ModuleDeclarationSite>`, so downstream code that builds `SyntaxFacts` with a struct literal stops compiling; `SyntaxFacts::default()` and `..Default::default()` are unaffected, and serialized data without the field still deserializes (#453).
- `open_kioku_core::SyntaxFacts` gains the public field `module_declarations: Vec<ModuleDeclarationSite>`, and `open_kioku_semantic_model::ImportBinding` gains `rule: ImportBindingRule`, so downstream code that builds either with a struct literal stops compiling; `SyntaxFacts::default()` and `..Default::default()` are unaffected, and serialized data without the fields still deserializes (#453).
