...
- A repository that has never been indexed is served, not silently indexed. `ok mcp serve` opened the index store with `CREATE`, so the first tool call on a never-indexed repository wrote an empty `.ok/index.sqlite` even under `--read-only`, `repo_status` answered `null`, `search_code` answered an empty success, `build_context_pack` blamed the task, and `get_references` called the empty file a legacy index. Every read surface now opens through `SqliteStore::open_existing`/`open_repo_index`, which create nothing; `SqliteStore::open` remains the writers' open. Without an index the MCP server still answers `initialize` and `tools/list`, `repo_status` returns `{indexed: false, index_path, message, next_step}` (`indexed: true` is added to the indexed answer), and every other tool call is a JSON-RPC error whose message says the repository is not indexed and names `ok index <repo>`; an index built while the server is running is picked up on the next request. `ok status`, `ok --json status`, `ok doctor`, `ok search`, `ok context`, `ok impact` and `ok plan` say the same sentence, and `ok doctor`'s MCP probe no longer creates the index it then reports on. A database without a manifest is treated as unindexed, so the file 4.0.0 left behind gets the same answer instead of the "legacy index" rebuild message, which remains reserved for a real pre-4.0 index.
- `ok setup agent --apply` reconciles the client configuration before indexing. It indexed first and then refused an `open-kioku` entry it had not written, so a hand-written `.mcp.json` (what `ok mcp install claude` prints) cost a full index and failed anyway, and `--check` recommended the same `--apply`. An existing entry that launches `ok mcp serve` for this repository is now adopted as-is and reported as `[kept] config … existing entry preserved`, leaving a tracked `.mcp.json` untouched; an entry that launches something else or serves another repository is refused before anything is indexed or written, with the `mcpServers.open-kioku` key and the exact value that would be compatible. `--check` reports `[mismatch]` with that edit for a conflicting entry or a foreign guidance file and no longer recommends `--apply` on a state `--apply` refuses.
- Context packs rendered as Markdown, text, or TOON print `no token ceiling; file limit N` for the file-limit budget instead of the `4611686018427387903` sentinel that `ContextBudget::from_file_limit` stores in `max_tokens`. `ContextBudget::has_token_ceiling` is the check renderers use. The JSON field is unchanged: it is still the documented sentinel.
- Four MCP inputs that succeeded silently now say what is missing. `impact_analysis` on a path the index does not hold names it in `risk_report.reasons` and in its evidence message instead of reporting `limited indexed downstream references found`; `search_code` refuses a blank `query`; `dependency_path` refuses a `from` or `to` that resolves to no indexed file, symbol, or graph node instead of returning an empty edge list; `retrieve_context` refuses an unknown handle instead of returning `null`. `ok search`, `ok path` and `ok retrieve-context` apply the same checks.
- The `ok.toml` written by `ok init` and `ok setup agent --apply` prints the ranking weights as declared (`graph_proximity = 0.35`, not `0.3499999940395355`; the `f32` weights now serialize as their shortest round-trip decimal everywhere), leaves `[runtime]` at `enabled = false` with the provider fields as comments instead of a pre-filled `provider = "sentry"`, and carries a comment above `resolution_mode` describing `legacy`, `shadow` and `v2`. The loader reads the file back as the defaults.
