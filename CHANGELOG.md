...
- Context-pack and plan confidence is derived from evidence provenance, not from result prose. `exact_reference_count` was granted by a substring test for `scip`/`exact reference` over every result's evidence lines and match reason, and the lexical stream's `query variant `...` matched local index` line quotes the query words, so a target file defining `scip_setup_report` made `ok context` report `Exact (1.00)` beside `exact_evidence_count: 0`; the `runtime` corroboration count leaked the same way. Both now count typed sources only: exact-authority retrieval traces (the same set `selection.exact_evidence_count` reports), the impact engine's own symbol-occurrence results (`open_kioku_impact::is_exact_reference_result`), `source_type: scip` evidence, and the `runtime_corroboration` score component. `Exact` is reachable only with `exact_reference_count > 0` and otherwise caps at `High`. `negative_evidence_count` was a second substring test over `risk.reasons`, which only `ok plan` fed with evidence-quality caveats, so plan reported `3 negative evidence signal(s)` and capped at 0.60 where `ok context` counted 0 for the same task; both now count the `primary_context` and `anchor` items of the `negative_evidence` list they publish. Named task identifiers that no selected context spells (`FrobnicateWidgetManager`) are `anchor` negative evidence on both surfaces, using `open_kioku_core::named_anchors` lifted from the plan crate; when every named identifier is unmatched the score caps at 0.50 and a blocker names them. `evidence_density` counts distinct evidence records instead of evidence lines, and the `validation_availability`/`test_coverage` rationales say what they measure. The Markdown line `Exact-evidence selections` is now `Exact-authority selections` so unit provenance is not read as the pack's label. `docs/context-pack-spec.md` defines the label semantics and `docs/ranking.md` the components.
