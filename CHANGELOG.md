...
### Added
- `ok search <pattern> --regex` runs exact regular-expression line matching over the indexed corpus. It makes the same call the MCP `regex_search` tool now makes, so the two surfaces answer identically.
