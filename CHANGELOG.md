...
- `open-kioku-core`: `IndexQuality` gains `redacted_files: Option<usize>`; a struct literal that builds `IndexQuality` must set it or end with `..Default::default()`.
...
- Files in a data, config, or prose format (YAML, JSON, TOML, Markdown, plain text, and document-corpus files) are indexed with secret-like values replaced by `[REDACTED]` before chunks, symbols, analysis facts, test candidates, and document sections are derived from them, so the values are absent from `.ok/index.sqlite`, the Tantivy index, `ok snapshot export` artifacts, `ok search`, and MCP results; line numbers are unchanged. A value is replaced when its key names a secret, when it is a private-key PEM block or a URL password, or, in YAML, JSON, TOML, Terraform/HCL, and Dockerfiles but not in Markdown or plain text, when it is a machine-generated-looking token of at least 20 characters; algorithm-labelled digests such as lockfile `sha512-` integrity values, `null`, booleans, unquoted numbers, and bare `${VAR}` references are kept. The semantic vector store is built from the same redacted chunks. Rules, calibration, and limits: `docs/security-model.md`, "Secret-value redaction". Programming-language source is indexed as written. `ok index` and `ok status` print the number of files with redacted values, `ok doctor` gains a `redaction` check (a warning on an index written before redaction, with `ok index .` as the next step), the `ok status --markdown` table gains a `Files with redacted values` row, and `ok --json status` and `repo_status` carry the count as `quality.redacted_files`, absent on older manifests. The first `ok index` over an index written before redaction runs `VACUUM` on `.ok/index.sqlite` after publishing and truncates the WAL, so free pages holding values the earlier release stored as read are dropped; this is a one-time rewrite of the database that takes time proportional to its size and needs free disk space about its size; it runs after the manifest is published, so a reader holding the database defers it to the next run rather than failing one. `ok watch` rebuilds such an index in full instead of updating it in part, then compacts it the same way (#379).
