...
- `commit-derived-bench.yml` and `semantic-experiment.yml` read each corpus's repository URL and base commit from the `BENCH_<CODE>_URL` and `BENCH_<CODE>_BASE` repository secrets and mask the forms of them a log can still show (`scripts/mask-corpus-identity.sh`). Manual runs no longer accept a repository URL, base commit, or corpus name. `scripts/commit-derived-cases.py --quiet` prints counts only and withholds failure details, and the uploaded artifacts carry aggregate results only: `scripts/reduce-benchmark-report.py` drops per-case rows and per-directory coverage after the baseline comparison, and `cases.tsv` is not uploaded. This keeps corpus identity out of public logs and artifacts.
