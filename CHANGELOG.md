...
- Import-bound call, declared-type and inheritance resolution in every language considers only the import bindings in scope at the use: those declared in the same scope, an enclosing scope, or at no recorded scope. An import inside one function or `mod tests` block no longer binds a call or type use elsewhere in the file, and a module-level call in the same scope as its import can now bind through it. When any in-scope import of the name is unresolved, that import produces no candidate, and resolution falls back to the same-file and qualified-name rules (#453).
