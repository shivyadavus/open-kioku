...
- The first `ok watch` event on an index written with `IndexManifest.schema_version` 2 (4.0.0) re-indexes the repository in full: `partial_index_supported` requires the stored and current versions to match, and the current version is 3.
...
- Test targets are functions, methods, and test symbols with a test annotation (`#[test]`, `#[tokio::test]`, `#[rstest]`, `#[test_case]`, `@Test`, `@ParameterizedTest`, `it(`, `test(`, `def test_`) on their first line or the three lines above it, or any symbol in a test-path file. `extract_tests` scanned the whole file for `#[test]`, so a file with an inline `#[cfg(test)] mod tests` made every constant and struct in it a validation target. Targets are persisted at index time: run `ok index` to drop the stale ones.
- Context-pack evidence carries one id per fact: each retrieval evidence line of a primary result takes the `search:<path>:<range>:<index>` id its result's `evidence_refs` already cite, instead of `context:<path>` shared by every line of that file. Records are taken before runtime and history annotation append lines; a result's refs name its lines only when there is one ref per line, and otherwise each line is published under its derived id, so no line is published under another fact's id. Refs of results merged from several chunks or retrieval streams are not index-aligned with their lines and can still cite ids with no record in the pack. Runtime and git-history facts resolve to their producers' records. Pack and plan evidence are deduplicated by id, and `validation_plan.evidence` holds only the records its tests cite instead of a second copy of the pack's evidence.
- `impact_analysis`, `ok impact`, context packs and plans return one direct impact per path per edge kind (exact reference, co-change, runtime, service boundary, lexical) instead of one per matching chunk. The best-scoring chunk is the entry; the evidence lines of up to four other chunks are kept under their own ids, prefixed with `lines <start>-<end>` and the symbol, and one further line names the line ranges of any remaining chunks. The 25-entry limit now bounds files rather than chunks, and the Markdown and prompt-text context renderings list each supporting path once.
- `ok verify` and `verify_change` list the symbols whose ranges overlap a diff's `@@` hunks, innermost per hunk, instead of every symbol in each touched file; `VerifyChangeInput.changed_ranges` carries ranges for callers without a diff. Hunks no symbol covers are reported under `changed_regions_without_symbol`. With only changed file paths, symbols stay file-level and a `symbol_granularity` warning names each path without changing the verdict.
- A verification request with no change is reported as invalid input, `OkError::InvalidInput`, instead of a configuration error: `ok verify` without `--git`, `--diff`, `--changed` or `--since-plan` is a usage error with exit code 2, an empty diff exits 2 with `invalid input: …`, and `verify_change` returns JSON-RPC `-32602` instead of `-32000`. Invalid contract input on contract verification uses the same variant.
