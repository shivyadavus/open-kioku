...
### Fixed
- The indexer no longer drops source files whose path contains "secret" or "credential" (25 Java files vanished from one repository), and no longer skips generated files: they are indexed, flagged `is_generated`, ordered behind other candidates before each stream's cap, and ranked at the lowest quality tier (`generated_file_demotion`) unless the task names the file's own path. Data, config and prose files keep the secret-name rule (#379). py-a holdout against the same index, measured on the production `ok context` path with `scripts/score-context-cases.py`: R@5 0.645 → 0.665, R@20 0.741 → 0.767.
