...
- A commit scope's directory entry file (`mod.ts`, `index.ts`, `lib.rs`, `__init__.py`) is a candidate even without shared vocabulary, placed just below the scope's best hit when nothing in the directory knows the task's words and last otherwise. ts-a holdout R@5 0.744 → 0.816, MRR 0.607 → 0.650; other corpora unchanged.
- Commit-style scope prefixes (`docs(fs): …`, `pkg/render: …`, `[Scheduler] …`) now anchor retrieval on the package or directory they name, and document sections cast one vote per file instead of one per section (a documentation task on ts-a had returned twenty sections of `Releases.md` and nothing else). Holdout MRR on the commit-derived corpora: ts-a 0.510 → 0.607, go-a 0.308 → 0.338, py-a 0.539 → 0.565.
...
### Changed
- The Tantivy index tokenizes code text and symbols identifier-aware: `FieldMapper` is indexed as `fieldmapper`, `field`, and `mapper`. Lexical MRR on a 490-case commit-derived java-a benchmark: 0.235 → 0.393 (dev), 0.202 → 0.337 (holdout). Existing indexes keep working; run `ok index` to rebuild with the new tokenizer.
...
### Added
- `scripts/commit-derived-cases.py` keeps one instance of a subject that repeats up to numbers (`--keep-repeated-subjects` restores the old behaviour); go-a's corpus shrinks from 481 to 280 cases because 201 were release-bump commits with one gold file.
...
- **SQLite Ingestion & Backfill Performance**:
  - Wrapped SQLite node and edge backfill updates (`backfill_graph_query_columns`) inside transactions. This critical performance fix reduces backfill time on large codebases (such as java-a with 269k stale edges) from several hours to under 25 seconds.
...
- Evolved homepage to highlight plan-before-edit paradigm and show real java-a proof numbers.
...
- Added large-repo proof documentation for a local java-a validation run.
