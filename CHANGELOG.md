...
- `ok plan` and `plan_change` count a proven dependent in `impact.proven_impact` toward `exact_reference_count` when it is in a file other than the impact target, authoritative, and unambiguous, so a plan whose only exact evidence is a proven cross-file caller no longer reports `exact symbol/reference evidence is unavailable`; same-file edges such as `USES_TYPE` and ambiguous edges still do not count.
- An all-lowercase task word whose only separator is `-` (`re-index`, `drive-by`, `best-effort`) no longer produces the `task identifier(s) name nothing` blocker or its 0.50 cap in `ok context`, `ok plan`, `build_context_pack` and `plan_change`; when no selected context spells it, it stays in the `anchor` negative evidence and is named in the caveat `N hyphenated task word(s) appear in no selected context`. Such a word counts as a task identifier when quoted in backticks or when it contains a capital, a digit or `_`.
