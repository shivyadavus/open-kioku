...
- `open-kioku-core`: `ConfidenceSignalInput` gains `coverage_gaps: Vec<CoverageGap>` and `primary_language_keys: Vec<String>`. A struct literal without `..Default::default()` must add both; empty vectors keep the previous scoring.
