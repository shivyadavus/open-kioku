...
- `open-kioku-core` adds `weak_named_anchors`, and `ConfidenceSignalInput` gains the field `weak_anchors`, which a caller building the struct without `..Default::default()` must set. `named_anchors` no longer returns hyphenated task words.
...
- `ok plan` and `plan_change` count a proven dependent in `impact.proven_impact` toward `exact_reference_count` when it is in a file other than the impact target, authoritative, and unambiguous; same-file edges such as `USES_TYPE` and ambiguous edges still do not count, and `ok context` does not count proven dependents. A plan whose only exact evidence is such an edge no longer reports `exact symbol/reference evidence is unavailable`: `evidence_quality.exact_reference_available` becomes true, the `exact_references` negative evidence item is dropped, `exact_references` scores 1.00 instead of 0.25, and the 0.74 cap for plans without exact evidence no longer applies, so such a plan can read High instead of Medium.
